...
<div class="max-w-5xl mx-auto p-4 sm:p-6">
    <div class="mb-6">
        <a href="{{ URL::previous(route(auth()->check() ? 'live.index' : 'landing')) }}" class="text-sm text-gray-400 hover:text-white transition-colors inline-flex items-center gap-1">
    ...
    <div class="bg-surface-800 border border-white/10 rounded-xl p-6 mb-6">
        <div class="flex items-start justify-between gap-4">
            <div>
                ...
                <p class="text-gray-400 text-sm mt-1">{{ $session->course->title ?? __('General') }}</p>
                <div class="flex items-center gap-3 mt-3 text-sm text-gray-500">
                    <span class="flex items-center gap-1.5">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        {{ $session->scheduled_at ? $session->scheduled_at->format('M d, Y g:i A') : __('Not scheduled') }}
                    </span>
                    <span class="text-gray-600">&middot;</span>
                    <span class="capitalize">{{ $session->provider }}</span>
                </div>
                @if($session->scheduled_at && $session->scheduled_at->isFuture())
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-500/20 text-emerald-400 mt-3">{{ __('Upcoming') }}</span>
                @elseif($session->scheduled_at)
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-500/20 text-gray-400 mt-3">{{ __('Past') }}</span>
                @else
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-500/20 text-gray-400 mt-3">{{ __('Unscheduled') }}</span>
                @endif
            </div>
        </div>
    </div>
    ...
    @elseif($session->room_url && $session->scheduled_at && $session->scheduled_at->isFuture())
        <div class="bg-surface-800 border border-white/10 rounded-xl overflow-hidden mb-6">
            <div class="p-4 border-b border-white/10 flex items-center justify-between">
                <span class="text-sm font-medium text-white">{{ __('Live Room') }}</span>
    ...
    @elseif($session->recording_url)
        <div class="bg-surface-800 border border-white/10 rounded-xl overflow-hidden mb-6">
            <div class="p-4 border-b border-white/10">
                <span class="text-sm font-medium text-white">{{ __('Recording') }}</span>
            </div>
            <div class="p-6">
    ...
    @else
        <div class="bg-surface-800 border border-white/10 rounded-xl p-10 text-center">
            ...
            <p class="text-gray-400 font-medium">{{ __('No room or recording available') }}</p>
            <p class="text-gray-600 text-sm mt-1">{{ __('Check back closer to the scheduled time.') }}</p>
        </div>
    @endif
</div>
