<x-layouts.dashboard>
    <x-slot name="title">{{ __('Create Lesson') }} - {{ $course->title }}</x-slot>
    ...
    <div class="bg-surface-800 border border-surface-700 rounded-xl p-6 max-w-2xl">
        <h1 class="text-xl font-bold text-white mb-6">{{ __('Create Lesson') }}</h1>
        <form method="POST" action="{{ route('courses.content.lesson.store', $course) }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            <input type="hidden" name="module_id" value="{{ $module->id }}">

            <div>
                <label class="block text-sm font-medium text-gray-300 mb-1.5">{{ __('Module') }}</label>
            ...
            <div>
                ...
                <textarea name="content" rows="8"
                          class="w-full bg-surface-700 border border-surface-600 rounded-lg px-4 py-2.5 text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-colors"></textarea>
            </div>
            ...
            <div class="flex items-center gap-3 justify-end pt-2">
                ...
                <button type="submit" class="bg-brand-500 hover:bg-brand-600 text-white rounded-lg px-6 py-2 text-sm font-medium transition-colors">{{ __('Create Lesson') }}</button>
            </div>
        </form>
    </div>
</x-layouts.dashboard>
