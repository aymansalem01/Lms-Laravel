<x-layouts.dashboard>
    <x-slot name="title">{{ __('Gradebook') }} — {{ $assignment->title }}</x-slot>

    <div class="mb-6">
        <a href="{{ route('courses.assignments.show', [$course, $assignment]) }}" class="text-sm text-gray-400 hover:text-brand-300 transition-colors flex items-center gap-1.5 mb-4">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            {{ __('Back to assignment') }}
        </a>
        <h1 class="text-2xl font-bold text-white">{{ $assignment->title }}</h1>
        <p class="text-sm text-gray-400 mt-1">{{ $course->name }} — {{ __('Gradebook') }}</p>
    </div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-surface-800 border border-white/10 rounded-xl p-4">
            <p class="text-xs text-gray-500 uppercase tracking-wider">{{ __('Average Score') }}</p>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-left text-gray-500 text-xs uppercase tracking-wider border-b border-white/10">
                        ...
                        <th class="px-5 py-3">{{ __('Plagiarism') }}</th>
                        ...
                        <tr class="hover:bg-surface-700/50 transition-colors">
                            ...
                            <td class="px-5 py-3.5">
                                ...
                                @elseif($status === 'pending_review')
                                    <span class="text-[11px] font-semibold px-2 py-0.5 rounded-md bg-yellow-500/10 text-yellow-400">{{ __('Pending Review') }}</span>
                                @else
                            ...
                            <td class="px-5 py-3.5">
                                @if($grade)
                                    <span class="text-gray-300 font-medium">{{ number_format($grade->score, 1) }} / {{ $assignment->max_score }}</span>
                                @else
                                    <span class="text-gray-600">—</span>
                                @endif
                            </td>
                            <td class="px-5 py-3.5">
                                @if($submission && $submission->plagiarismReport)
                                    @if($submission->plagiarismReport->similarity_score > 70)
                                        <span class="text-[11px] font-semibold px-2 py-0.5 rounded-md bg-red-500/10 text-red-400">{{ __('High') }}</span>
                                    @elseif($submission->plagiarismReport->similarity_score > 30)
                                        <span class="text-[11px] font-semibold px-2 py-0.5 rounded-md bg-yellow-500/10 text-yellow-400">{{ __('Medium') }}</span>
                                    @else
                                        <span class="text-[11px] font-semibold px-2 py-0.5 rounded-md bg-green-500/10 text-green-400">{{ __('Low') }}</span>
                                    @endif
                                @else
                                    <span class="text-gray-600">—</span>
                                @endif
                            ...
                            <td class="px-5 py-3.5 text-right">
                                <button type="button"
                                        ...
                                        class="text-sm bg-brand-600 hover:bg-brand-500 text-white font-medium rounded-xl px-4 py-1.5 transition-colors duration-200">
                                    {{ __('Grade') }}
                                </button>
                            </td>
                        </tr>
