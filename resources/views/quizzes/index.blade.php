    ...
    @else
        <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            @foreach($quizzes as $quiz)
                @php
                    $isInstructor = auth()->user()->isInstructorOrAdmin();
                    $grade = !$isInstructor ? $quiz->computedGrade() : null;
                ...
                <a href="{{ route('courses.quizzes.show', [$course, $quiz]) }}"
                   class="bg-surface-800 border border-white/10 rounded-xl p-5 hover:border-brand-500/50 hover:bg-surface-750 transition-all duration-200 group block">
                    <div class="flex items-start justify-between gap-4 mb-3">
                        <h3 class="text-white font-semibold group-hover:text-brand-300 transition-colors truncate">{{ $quiz->title }}</h3>
                        <div class="flex items-center gap-2 shrink-0">
                            <span class="text-[11px] font-semibold px-2 py-0.5 rounded-md bg-{{ $badge[1] }}-500/10 text-{{ $badge[1] }}-400">{{ $badge[0] }}</span>
                    ...
                    <div class="flex items-center gap-4 text-xs text-gray-500">
                        ...
                        @if(!$isInstructor && $grade)
                            <span class="flex items-center gap-1.5 text-brand-400 font-medium ml-auto">
