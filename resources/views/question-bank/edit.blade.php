<x-layouts.dashboard>
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-400 mt-1">{{ $course->name }} &middot; {{ $questionBankItem->bank->name }}</p>
    </div>
        ...
        <form method="POST" action="{{ route('courses.question-bank.update', [$course, $questionBankItem]) }}" class="space-y-6">
            ...
            <div class="bg-surface-800 border border-white/10 rounded-2xl p-6 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label for="points" class="block text-sm font-medium text-gray-300 mb-1.5">{{ __('Points') }}</label>
                ...
                <div>
                    ...
                    <textarea name="question" id="question" rows="3" required
                              ...
                              placeholder="{{ __('Enter your question...') }}">{{ old('question', $questionBankItem->question) }}</textarea>
                </div>
                ...
                <div id="bank-options-container" class="{{ $questionBankItem->type !== 'multiple_choice' ? 'hidden' : '' }}">
                    ...
                    <div class="flex items-center gap-2 mb-2">
                        <input type="text" name="options[]"
                               ...
                               placeholder="{{ __('Option') }} {{ chr(65 + $opt) }}" value="{{ old("options.$opt", $itemOptions[$opt] ?? '') }}">
