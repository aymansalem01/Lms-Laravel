<x-layouts.dashboard>
    <x-slot name="title">Edit Question — SAE LMS</x-slot>
    ...
    <div class="max-w-3xl">
        <form method="POST" action="{{ route('admin.question-bank.update', $questionBankItem) }}" class="space-y-6">
            @csrf @method('PUT')
            ...
            <div class="bg-surface-800 border border-white/10 rounded-2xl p-6 space-y-6">
                <div>
                    ...
                    <select name="course_id" id="course_id" class="input-dashboard">
                        @foreach($courses as $course)
                            <option value="{{ $course->id }}" {{ old('course_id', $questionBankItem->course_id) == $course->id ? 'selected' : '' }}>{{ $course->title }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="type" id="type" onchange="toggleBankOptions(this.value)" class="input-dashboard">
                    ...
                    <div>
                        ...
                        <input type="number" name="points" id="points" value="{{ old('points', $questionBankItem->points) }}" min="1" class="input-dashboard">
                    </div>
                </div>

                <div>
                    <label for="question" class="block text-sm font-medium text-gray-300 mb-1.5">Question</label>
                    <textarea name="question" id="question" rows="3" required class="input-dashboard resize-none">{{ old('question', $questionBankItem->question) }}</textarea>
                </div>
                ...
                <div id="bank-options-container">
                    ...
                    <div class="flex items-center gap-2 mb-2">
                        <input type="text" name="options[]" class="flex-1 bg-surface-800 border border-white/10 text-white rounded-xl py-2.5 px-3 text-sm focus:outline-none focus:border-brand-500" placeholder="Option {{ chr(65 + $optIndex) }}" value="{{ $option }}">
                        <label class="flex items-center gap-1.5 text-xs text-gray-500 shrink-0">
                            <input type="radio" name="correct_answer" value="{{ $optIndex }}" class="accent-brand-500" {{ $questionBankItem->correct_answer !== null && (int)$questionBankItem->correct_answer === $optIndex ? 'checked' : '' }}> Correct
                        </label>
                    </div>
                    @endforeach
                </div>
                ...
                <div id="bank-true-false-container" class=" space-y-2">
                    ...
                    <div class="flex items-center gap-4">
                        <label class="flex items-center gap-2 text-sm text-gray-300"><input type="radio" name="correct_answer" value="true" class="accent-brand-500" {{ $questionBankItem->correct_answer === 'true' ? 'checked' : '' }}> True</label>
                        <label class="flex items-center gap-2 text-sm text-gray-300"><input type="radio" name="correct_answer" value="false" class="accent-brand-500" {{ $questionBankItem->correct_answer === 'false' ? 'checked' : '' }}> False</label>
                    </div>
                </div>
                ...
                <div id="bank-short-answer-container" class=" space-y-2">
                    ...
                    <input type="text" name="correct_answer" id="correct_answer_short" class="input-dashboard" value="{{ old('correct_answer', $questionBankItem->type === 'short_answer' ? $questionBankItem->correct_answer : '') }}">
                </div>
            </div>

            <div class="flex items-center gap-3">
                <button type="submit" class="bg-brand-600 hover:bg-brand-500 text-white font-semibold rounded-xl px-6 py-2.5 transition-colors duration-200">Update Question</button>
