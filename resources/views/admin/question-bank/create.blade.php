<x-layouts.dashboard>
    <x-slot name="title">Add Question — SAE LMS</x-slot>
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-white">Add Question to Bank</h1>
    </div>
            ...
            <div class="bg-surface-800 border border-white/10 rounded-2xl p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="points" id="points" value="{{ old('points', 10) }}" min="1" class="input-dashboard">
                    </div>
                </div>

                <div>
                    <label for="question" class="block text-sm font-medium text-gray-300 mb-1.5">Question</label>
                    <textarea name="question" id="question" rows="3" required class="input-dashboard resize-none" placeholder="Enter your question...">{{ old('question') }}</textarea>
                </div>
                ...
                <div id="bank-options-container">
                    ...
                    <div class="flex items-center gap-2 mb-2">
                        <input type="text" name="options[]" class="flex-1 bg-surface-800 border border-white/10 text-white rounded-xl py-2.5 px-3 text-sm focus:outline-none focus:border-brand-500" placeholder="Option {{ chr(65 + $opt) }}" value="{{ old("options.$opt") }}">
                        <label class="flex items-center gap-1.5 text-xs text-gray-500 shrink-0">
                            <input type="radio" name="correct_answer" value="{{ $opt }}" class="accent-brand-500" {{ old('correct_answer') == $opt ? 'checked' : '' }}> Correct
                        </label>
                    </div>
                    @endfor
                </div>
                ...
                <div id="bank-true-false-container" class=" space-y-2">
                    ...
                    <div class="flex items-center gap-4">
                        <label class="flex items-center gap-2 text-sm text-gray-300"><input type="radio" name="correct_answer" value="true" class="accent-brand-500" {{ old('correct_answer') === 'true' ? 'checked' : '' }}> True</label>
                        <label class="flex items-center gap-2 text-sm text-gray-300"><input type="radio" name="correct_answer" value="false" class="accent-brand-500" {{ old('correct_answer') === 'false' ? 'checked' : '' }}> False</label>
                    </div>
                </div>
                ...
                <div id="bank-short-answer-container" class=" space-y-2">
                    ...
                    <input type="text" name="correct_answer" id="correct_answer_short" class="input-dashboard" value="{{ old('correct_answer') }}" placeholder="Expected answer...">
                </div>
            </div>

            <div class="flex items-center gap-3">
                <button type="submit" class="bg-brand-600 hover:bg-brand-500 text-white font-semibold rounded-xl px-6 py-2.5 transition-colors duration-200">Add to Bank</button>
