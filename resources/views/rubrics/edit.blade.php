<x-layouts.dashboard>
    <x-slot name="title">{{ __('Edit Rubric') }} — {{ $rubric->title }}</x-slot>

    @php
        $rubricData = $rubric->toArray();
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-white">{{ __('Edit Rubric') }}</h1>
        <p class="text-sm text-gray-400 mt-1">{{ $rubric->title }}</p>
    </div>

    <div class="max-w-5xl">
        <form method="POST" action="{{ route('courses.rubrics.update', [$course, $rubric]) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div x-data="rubricBuilder()" x-init="init()" class="space-y-6">
                ...
                <div class="bg-surface-800 border border-white/10 rounded-2xl p-6">
                    ...
                    <p class="text-xs text-gray-500 mt-3">{{ __('Enter scores for each criterion-level combination.') }}</p>
                </div>
            </div>
